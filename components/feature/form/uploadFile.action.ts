...
export async function uploadFile(file: File, name: string) {
    ...
    if (!userId) {
        return { success: false, message: "err1 Une erreur s'est produit lors de l'upload" }
    }
	...
	if (!file.type.startsWith("image/")) {
		...
		try {
			...
			return { success: false, message: "err2 Une erreur s'est produit lors de l'upload" }
		}
	...
	try {
		...
		return { success: false, message: "err3 Une erreur s'est produit lors de l'upload" }
	}
